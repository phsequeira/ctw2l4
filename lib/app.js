...
app.get('/weather/16Days', async(req, res) => {
  try {
    const city = req.query.city;
    const state = req.query.state;
    const country = req.query.country;

    const weatherData = await request.get(`https://api.weatherbit.io/v2.0/forecast/daily?city=${city},${state},${country}&key=${process.env.weatherkey}`);
